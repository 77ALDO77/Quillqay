...
export default function NotesSection() {
  ...
  const handleCreate = () => {
    ...
    setNotes([{ id: `note-${Date.now()}`, text: newText.trim(), color: noteColors[Math.floor(Math.random() * 3)], createdAt: 'Just now' }, ...notes]);
    setNewText('');
    setShowNew(false);
  };
  ...
  return (
    <SectionShell title="Notes" description="Quick ideas, todos, and reminders for this project." actionLabel="New Note" onAction={() => setShowNew(true)}>
      {notes.length === 0 ? (
              ...
              {editingId === note.id ? (
                ...
                <div className="p-4 cursor-pointer" onClick={() => { setEditingId(note.id); setEditText(note.text); }}>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setNotes(notes.filter((n) => n.id !== note.id)); }} className="p-1 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-error/10 transition-all">
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="glass-panel relative z-10 w-full max-w-md rounded-3xl border border-white/10 shadow-2xl p-8">
            <div className="flex items-center justify-between mb-6"><h2 className="text-lg font-bold">New Note</h2><button onClick={() => setShowNew(false)} className="p-2 rounded-full hover:bg-white/5"><X className="w-5 h-5 text-on-surface-variant" /></button></div>
